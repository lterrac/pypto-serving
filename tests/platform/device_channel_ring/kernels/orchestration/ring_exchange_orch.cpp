...
ring_exchange_orchestration_config(const L2TaskArgs &orch_args) {
    (void)orch_args;
    return PTO2OrchestrationConfig{.expected_arg_count = 6};
}

__attribute__((visibility("default"))) void ring_exchange_orchestration(const L2TaskArgs &orch_args) {
    const Tensor &input = orch_args.tensor(0).ref();
    const Tensor &output = orch_args.tensor(1).ref();
    const Tensor &payload = orch_args.tensor(2).ref();
    const Tensor &signal = orch_args.tensor(3).ref();

    L0TaskArgs params;
    params.add_input(input);
    params.add_output(output);
    params.add_inout(payload);
